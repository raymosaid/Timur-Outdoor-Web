...
const DatePickerWithRange = React.forwardRef(
  (
    ...
    const [fromMonth, setFromMonth] = React.useState(value ?? defaultPopupValue);
    const [toMonth, setToMonth] = React.useState(value ?? defaultPopupValue);
     ...
     */
    const handleFromMonthChange = (newDay) => {
      if (!newDay) {
        return;
      }
      ...
      setFromMonth(newDateFull);
    };

    const handleToMonthChange = (newDay) => {
      if (!newDay) {
        return;
      }
      if (!defaultPopupValue) {
    ...
    return (
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="grid grid-cols-2 w-auto p-0">
          <Calendar
            mode="single"
